...
def send():
    ...
    cap_send = cv2.VideoCapture("rtspsrc location=rtsp://184.72.239.149/vod/mp4:BigBuckBunny_115k.mov ! application/x-rtp, media=(string)video \
    ! decodebin    ! video/x-raw, format=(string)NV12 \
    ! videoconvert ! video/x-raw, format=(string)BGR \
	! appsink", cv2.CAP_GSTREAMER)
    out_send = cv2.VideoWriter('appsrc ! videoconvert ! x264enc tune=zerolatency bitrate=500 speed-preset=superfast ! rtph264pay ! udpsink host=192.168.0.20 port=5000',cv2.CAP_GSTREAMER,0, 20, (320,240), True)
